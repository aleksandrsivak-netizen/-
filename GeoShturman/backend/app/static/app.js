/* =========================================================================
   ...
   Работает с бэкендом (POST /api/demo/run) и автономно (demo-fallback).
   ========================================================================= */
"use strict";
...
(() => {
  ...
  function collectPayload() {
    ...
    return {
      ...
      noise_std_m: num("noise_std_m"),
      outlier_probability: num("outlier_probability"),
      search_radius_m: num("search_radius_m"),
  ...
  const STEP_LABELS = [
    "Получение данных радиовысотомера",
    "Построение профиля рельефа",
    "Генерация эталонных профилей",
    "Поиск совпадений (корреляция)",
    "Решение найдено",
  ];
  let stepTimer = null;
  ...
  function normalizeResponse(raw, payload) {
    raw = raw || {};
    const est = raw.estimated || raw.result || {};
    ...
    return {
      ...
      estimated: {
        azimuth_deg: pick(est.azimuth_deg, est.azimuth, payload.azimuth_deg),
        speed_mps: pick(est.speed_mps, est.speed, payload.speed_mps),
        start_x_m: pick(est.start_x_m, est.x, null),
        start_y_m: pick(est.start_y_m, est.y, null),
      },
      truth: {
        azimuth_deg: pick(truth.azimuth_deg, truth.azimuth, payload.azimuth_deg),
        speed_mps: pick(truth.speed_mps, truth.speed, payload.speed_mps),
        start_x_m: pick(truth.start_x_m, truth.x, payload.width_m / 2),
        start_y_m: pick(truth.start_y_m, truth.y, payload.height_m / 2),
      },
      metrics: {
        correlation: pick(m.correlation, m.corr, null),
        rmse_m: pick(m.rmse_m, m.rmse, null),
        confidence: pick(m.confidence, m.conf, null),
        confidence_pct: pick(m.confidence_pct, m.confidence != null ? m.confidence * 100 : null, null),
        cep_m: pick(m.cep_m, m.cep, null),
        sep_m: pick(m.sep_m, m.sep, null),
        vertical_m: pick(m.vertical_m, m.vertical, null),
        distance_km: pick(m.distance_km, raw.distance_km, null),
        offset_km: pick(m.offset_km, m.offset, null),
      },
      found_position: {
      ...
      profile: raw.profile || null, // { radio:[], dem:[] }
      path: raw.path || raw.trajectory || null, // [[x,y],...] доли 0..1 или метры
      artifacts: {
        trajectory_overlay_png: art.trajectory_overlay_png || art.map || null,
        correlation_heatmap_png: art.correlation_heatmap_png || art.heatmap || null,
        profile_comparison_png: art.profile_comparison_png || art.profile || null,
  ...
  function renderResults(data, payload) {
    ...
    setText("bm-corr", fmt(m.correlation, 3));
    ...
    if (data.profile) {
      $("#profileImg").hidden = !!art.profile_comparison_png;
    ...
    // --- Скачивание ---
    bindDownload("#dl-nmea", art.nmea_log, "track.nmea");
    bindDownload("#dl-json", art.result_json, "result.json");
    bindDownload("#dl-traj", art.trajectory_overlay_png, "trajectory.png");
    bindDownload("#dl-heat", art.correlation_heatmap_png, "heatmap.png");
    bindDownload("#dl-profile", art.profile_comparison_png, "profile.png");
  ...
  form.addEventListener("submit", async (ev) => {
    ...
    try {
      ...
      if (result.mode === "demo") {
        ...
        $("#formHint").textContent = "Получен ответ бэкенда.";
      }
    } catch (err) {
      setLoading(false);
      renderError(err);
